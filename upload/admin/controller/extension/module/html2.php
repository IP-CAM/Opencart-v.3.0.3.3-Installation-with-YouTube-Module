	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('html2', $this->request->post);
			} else {
...
/*
echo "<pre>";
print_r($data['module_description'] );

		foreach($data['module_description'] as $key =>  $value)
				{
				$corpusIn = '<p><iframe width="560" height="315" src="';
				$corpusOut = '" frameborder="0" allowfullscreen=""></iframe></p>';
				$k = $data['module_description'];
				$l = $k[1];
				$m = $l['beforename'];
				$beforename = $m;
				//print_r($m);
				$all = $corpusIn. $beforename. $corpusOut ;

				$data['module_description'][$key]['afterlink']  = $all;
				//array_push($data['module_description'][$key]['description'] , $link);
			}
			//$data['module_description'] = ($this->request->post['module_description']);
echo "<pre>";
print_r($data['module_description'] );
